...
import { GARMENT_CATEGORY_LABEL, DOLL_SIZE_LABEL, GARMENT_STATUS_LABEL } from "@/lib/constants";
...
const GarmentDetail = ({ garment }: Props) => {
  ...
  const lastScannedDate = new Date(garment.lastScannedAt).toLocaleDateString("ja-JP", {
    year: "numeric",
    month: "short",
    day: "numeric",
  });
      ...
      <Card>
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant={
                garment.status === "stored"
                  ? "confirmed"
                  : garment.status === "checked_out"
                    ? "uncertain"
                    : "unknown"
              }
            >
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-text-tertiary">最終スキャン: {lastScannedDate}</span>
            </div>
