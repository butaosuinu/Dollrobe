...
import CameraErrorOverlay from "@/components/scan/CameraErrorOverlay";
...
const QrScanner = ({ onScan, isActive }: Props) => {
  const { videoRef, cameraState, retry } = useQrCamera({ isActive });
  ...
  return (
    <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-black">
      ...
      {cameraState.status === "error" ? (
        <CameraErrorOverlay kind={cameraState.kind} onRetry={retry} />
      ) : (
